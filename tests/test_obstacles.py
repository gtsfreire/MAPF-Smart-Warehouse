...
def test_spawn_random_dynamic_creates_requested_obstacles():
    g = Grid(5, 5)
    om = ObstacleManager(g)

    created = om.spawn_random_dynamic(count=4, duration_range=(2, 2), kind="test")

    assert len(created) == 4
    assert len(om.dynamic_obstacles) == 4
    assert all(om.is_dynamic(x, y) for x, y in created)
    assert all(obs.duration == 2 for obs in om.dynamic_obstacles.values())
